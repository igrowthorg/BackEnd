...
export const MidwifeLogin = async (req, res, next) => {
  ...
  try {
    const [rows] = await pool.query(
      "SELECT midwife_id, area_id, email, nic FROM midwife WHERE nic = ? AND password = ? LIMIT 1",
      [nic, password]
    );
...
export const GetSDMeasurements = async (req, res, next) => {
  ...
  try {
    ...
    // Add data to arrays
    rows.forEach((row) => {
      if (row.months_difference >= 2 && row.months_difference <= 60)
    ...
    Object.keys(sixtyMonths_copy).map((key) => {
      if (sixtyMonths_copy[key].length > 0) {
        sixtyMonths_copy[key].forEach((row) => {
          ...
          else if (row.sd == "risk_of_under_weight")
            sd_count.risk_of_under_weight++;
          else if (row.sd == "minimum_under_weight")
            sd_count.minimum_under_weight++;
          else if (row.sd == "severe_under_weight")
            sd_count.severe_under_weight++;
...
export const GetVaccineTableForChild = async (req, res, next) => {
  ...
  try {
    ...
    all_vaccine.forEach((vaccine) => {
      ...
      const taken = take_vaccine.find(
        (take) => take.vaccine_id == vaccine.vaccine_id
      );
